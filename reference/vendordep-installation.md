...
across all supported hardware. Install each one from the same WPILib Vendor Dependencies panel as
YAGSL:
...
**ReduxLib** yourself from the same panel — it is not currently in YAGSL's auto-required list, even
though the parser supports Redux devices.
{% endhint %}

{% hint style="info" %}
If a vendor isn't in the panel's catalog on your WPILib version, fall back to
**Manage Vendor Libraries → Install new library (online)** with a URL from the vendor's own install
page — see the [WPILib guide to installing 3rd-party libraries](https://docs.wpilib.org/en/stable/docs/software/vscode-overview/3rd-party-libraries.html#installing-libraries).
{% endhint %}
